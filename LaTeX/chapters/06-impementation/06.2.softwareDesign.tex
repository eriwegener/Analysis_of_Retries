...
Der strukturelle Aufbau des Projekts ist in Abbildung~\ref{fig:softwareDesign} dargestellt und wird im
nachfolgenden Abschnitt detailliert beschrieben.
\input{figures/softwareDesign}

Der Programmablauf beginnt mit der Ausführung der \texttt{main}-Funktion.
Von dort aus werden nacheinander die Funktionen aus dem Modul \texttt{experimentRunner} und die
\texttt{start\_analysis}-Funktion aufgerufen.
...
Die im Modul \texttt{experimentRunner} implementierten Funktionen rufen die \texttt{start()}-Funktion im Modul
...
Diese wird dann von der Datenbank ausgeführt und diese gibt dann entweder den SLQSTATE-Code \texttt{00000} oder den
...
weil die Transaktion successful war, werden die Ergebnisse im JSONL-Format an die Funktion in
\texttt{experimentFunctions} zurückgegeben, wo diese dann in die Logdatei geschrieben werden.

Von dem allem separat ist das \texttt{analysis} Modul.
