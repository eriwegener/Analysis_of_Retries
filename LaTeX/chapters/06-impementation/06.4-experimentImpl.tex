...
Hierfür werden zwei Transaktionen verwendet, die jeweils eine Zeile der Tabelle \texttt{deadlocks} sperren und
anschließend versuchen, auf die von der anderen Transaktion gesperrte Zeile zuzugreifen.
Damit ein Deadlock entsteht, warten die Transaktionen 50\,ms aufeinander.
Der Einfluss dieser Wartezeit auf die Ergebnisse ist hingegen nachrangig, da diese durch den PostgreSQL-Default für
\texttt{deadlock\_timeout} (1\,s) dominiert werden.
Über die \texttt{client\_id} wird mit dem Modulo-Operator entschieden, welcher der beiden Clients welche Transaktion
ausführen soll.

\begin{lstlisting}[style=pythoncode, caption={Implementierung eines Deadlocks}, label={lst:deadlockImpl}]
def _tx_a(db):
    cur = db.conn.cursor()
...
Diese aufgerufene Funktion liest über den in Codeausschnitt~\ref{lst:caller-name} gezeigten Code den Namen von der
...
\begin{lstlisting}[style=pythoncode, caption={Ermittlung des Namens der aufrufenden Funktion}, label={lst:caller-name}]
