...
Allen Strategien liegt dabei ein gemeinsamer Ablauf zugrunde.
...
Während bei der \emph{immediate Retry}-Strategie der Delay nicht verwendet wird, wird bei \emph{Retry after Delay}
der Delay auf den Wert des Parameters gesetzt.
Die \emph{Retry with Backoff + Jitter}-Strategie wird hingegen berechnet, indem der initiale Delay mit $2^{n}$
multipliziert wird, wobei n für die Anzahl der bereits durchgeführten Retries entspricht.
...
Hierfür wird die Zeitdifferenz mit 1000 multipliziert und anschließend mit \texttt{math.floor} auf eine ganze Zahl
abgerundet.
