...
Die Funktion \texttt{start\_analysis()} wird durch die \texttt{main}-Funktion aufgerufen und startet die einzelnen
Analyseschritte.
...
Das \texttt{if hasattr} dient lediglich als Absicherung, dass nur für Strings die Zahl extrahiert wird, während bei
...
Die Quantile in \texttt{\_quantiles} berechnen sich analog dazu, außer dass \texttt{quantile([0.50, 0.95, 0.99, 0.999])}
anstelle von \texttt{mean} verwendet wurde.

Zur Bestimmung des durch den Retry-Mechanismus entstehenden Overheads werden die einzelnen Ausführungszeiten der
Transaktionsversuche (\texttt{attempts\_ms}) sowie die Gesamtdauer (\texttt{total\_ms}) betrachtet.
Da der erste Eintrag in \texttt{attempts\_ms} die ursprüngliche Ausführung der Transaktion repräsentiert und somit
keinen Retry darstellt, wird dessen Dauer von der Gesamtausführungszeit abgezogen.
Finden keine Wiederholungsversuche statt, beträgt der Overhead null.
Der Overhead ergibt sich somit direkt aus der Verzögerung und den Ausführungszeiten aller zusätzlichen Versuche
inklusive Wartezeiten.
Anschließend wird aus dem \texttt{overhead} der Durchschnitt berechnet.
Dadurch wird der tatsächliche zeitliche Zusatzaufwand bestimmt, der ausschließlich durch das Auftreten von Retries
verursacht wird.
