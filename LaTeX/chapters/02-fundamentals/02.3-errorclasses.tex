...
Für die Arbeit sind die folgenden SQLSTATE-Codes relevant:
\begin{itemize}
...
Dieses Level garantiert, dass gleichzeitig laufende Transaktionen kein Ergebnis erzeugen dürfen, das nicht entstehen
...
Wenn die Transaktion explizit ein Lock mit \texttt{SELECT ... FOR UPDATE} anfordert und dieses Lock aufgrund des
Timeouts nicht erhält, wird die Transaktion mit dem Code \texttt{55P03 lock not available} abgebrochen.

Deadlocks und Serialization Failures haben eine Sache gemeinsam, sie sind sogenannte \emph{transient Failures}.
Das bedeutet, dass die Fehler nur temporär sind und durch einen späteren Retry gelöst werden können.
Serialization Failures und Deadlocks können sich durch Warten vor der erneuten Ausführung der Transaktion lösen.
Wenn die abgebrochene Transaktion wartet, bis die andere erfolgreich abgeschlossen ist, dann kann bei nur zwei
Transaktionen kein erneuter Deadlock oder Serialization Failure mehr entstehen.
Deswegen sollten diese Fehlerklassen wiederholt werden~\cite{MicrosoftRetryPattern}.
Lock not available muss kein transient Failure sein, kann aber durch Retry gelöst werden, sodass diese Fehlerklasse
